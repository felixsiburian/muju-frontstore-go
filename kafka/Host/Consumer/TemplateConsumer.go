...
import (
	...
	"muju-frontstore-go/domain/repository/Template"
...
func consumeTemplate(topics []string, master sarama.Consumer) (chan *sarama.ConsumerMessage, chan *sarama.ConsumerError) {
	consumers := make(chan *sarama.ConsumerMessage)
	errors := make(chan *sarama.ConsumerError)
	fmt.Println("Kafka Template is Ready")
	//	//fmt.Println(topics)
		...
		go func(topic string, consumer sarama.PartitionConsumer) {
			for {
				...
				case msg := <-consumer.Messages():
					...
					tmp := model.Template{}
					switch msg.Topic {
					case "creates-template-topic":
						err := json.Unmarshal([]byte(msg.Value), &tmp)
						if err != nil {
							fmt.Println("Error : ",err.Error())
							os.Exit(1)
						}
						err = Template.CreateTemplate(&tmp)
						if err != nil {
							log.Fatal(err)
						}
						fmt.Println(string(msg.Value))
						fmt.Println("Template berhasil dibuat")
					case "updates-template-topic":
						...
						err = Template.UpdateTemplate(&tmp)
						if err != nil {
							log.Fatal(err)
						}
						fmt.Println(string(msg.Value))
						fmt.Println("Template berhasil di update")
					case "deletes-template-topic":
						...
						err = Template.DeleteTemplate(&tmp)
						if err != nil {
							log.Fatal(err)
						}
...
func NewTemplateConsumer() {
	...
	//
	consumer, errors := consumeTemplate(topics, master)
